...
// ─── Étapes génériques (fallback si l'IA ne répond pas) ─────────────────────
...
// ─── Hook : charger les bourses choisies depuis Payload ──────────────────────
...
// ─── Hook : générer/charger la roadmap d'une bourse via l'IA ─────────────────
function useRoadmap(bourse) {
  ...
  useEffect(() => {
    ...
    // Chercher d'abord dans le localStorage (cache 24h)
    ...
    const generate = async () => {
      ...
      try {
        ...
        if (rm && rm.etapes) {
          ...
          // Fallback : roadmap générique enrichie avec le nom de la bourse
...
function buildFallback(bourse) {
  return {
    ...
    etapes: [
      {
        id: 0, icon: '🎯', titre: 'Vérifier l\'éligibilité',
        description: `Vérifier que vous correspondez aux critères de ${bourse.nom}.`,
        deadline: 'Avant de commencer',
        documents: ['Relevés de notes', 'Passeport', 'Justificatifs de niveau'],
        conseils: [`Lisez attentivement les critères de ${bourse.nom}`, 'Contactez l\'ambassade si nécessaire'],
        dureeEstimee: '1 semaine',
      },
      {
        id: 1, icon: '📋', titre: 'Rassembler les documents',
        description: `Préparer le dossier complet selon les exigences de ${bourse.nom}.`,
        deadline: '6 semaines avant la deadline',
        documents: ['Relevés de notes traduits et apostillés', 'Diplômes certifiés', 'Passeport valide 6+ mois', 'Photos d\'identité'],
        conseils: ['Faites certifier chaque document', 'Prévoyez 2 copies de chaque pièce'],
        dureeEstimee: '3 semaines',
      },
      {
        id: 2, icon: '✍️', titre: 'Lettre de motivation',
        description: `Rédiger une lettre personnalisée pour ${bourse.nom} (${bourse.pays || ''}).`,
        deadline: '4 semaines avant',
        documents: ['Lettre de motivation (1-2 pages)', 'Plan de recherche si requis'],
        conseils: [`Mentionnez explicitement ${bourse.nom}`, 'Structure : projet → motivation → impact', 'Faites relire par un tiers'],
        dureeEstimee: '2 semaines',
      },
      {
        id: 3, icon: '📝', titre: 'CV académique',
        description: 'Adapter le CV au format international requis.',
        deadline: '4 semaines avant',
        documents: ['CV en français ET en anglais', 'Europass si requis'],
        conseils: ['Format 1-2 pages max', 'Inclure publications et prix', 'Adapter à la langue du pays'],
        dureeEstimee: '1 semaine',
      },
      {
        id: 4, icon: '🎙️', titre: "Préparer l'entretien",
        description: `S'entraîner avec le simulateur IA OppsTrack pour l'entretien ${bourse.nom}.`,
        deadline: '2 semaines avant',
        documents: ['Notes de préparation', 'Questions types de la bourse'],
        conseils: ['Utilisez le simulateur entretien OppsTrack', `Renseignez-vous sur ${bourse.pays || 'le pays cible'}`, 'Entraînez-vous en langue étrangère'],
        dureeEstimee: '2 semaines',
      },
      {
        id: 5, icon: '📬', titre: 'Soumettre la candidature',
        description: `Déposer le dossier complet sur la plateforme officielle de ${bourse.nom}.`,
        deadline: bourse.deadline || 'Voir site officiel',
        documents: ['Dossier complet finalisé', 'Formulaire de candidature rempli'],
        conseils: ['Soumettez 48h avant la deadline', 'Conservez les accusés de réception', 'Vérifiez chaque fichier avant envoi'],
        dureeEstimee: '2 jours',
      },
      {
        id: 6, icon: '🎉', titre: 'Suivi & résultats',
        description: 'Suivre l\'avancement et répondre aux demandes complémentaires.',
        deadline: 'Après soumission',
        documents: [],
        conseils: ['Vérifiez vos emails quotidiennement', 'Préparez un plan B', 'Restez disponible pour un 2ème entretien'],
...
// ─── Composant : une timeline pour une bourse ────────────────────────────────
      ...
      {isActive && (
        <div style={S.timelineBody}>
          ...
          {!genLoading && etapes.map((etape, i) => {
            ...
            return (
              <div key={etape.id ?? i} style={S.stepRow}
                ...
                <div style={{ ...S.stepContent, paddingBottom: i < etapes.length - 1 ? 24 : 8 }}>
                  <div style={S.stepHead}>
                    <div>
                      ...
                      {etape.deadline && (
                        <div style={S.deadline}>⏰ {etape.deadline}</div>
                      )}
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      {etape.dureeEstimee && (
                        <span style={S.dureeBadge}>{etape.dureeEstimee}</span>
                      )}
                      <span style={{ ...S.stepNum, color: isPending ? '#334155' : '#475569' }}>
                        {i + 1}/{total}
                      </span>
                    </div>
                  </div>
                    ...
                    <div style={S.docsBox}>
                      ...
                      {etape.documents.map((doc, j) => (
                        <div key={j} style={S.docItem}>
                          <span style={{ color: isCompleted ? '#10b981' : color }}>
                            {isCompleted ? '✓' : '→'}
                          </span>
                          <span>{doc}</span>
                        </div>
                  ...
                  {isActif && (
                    <div style={S.stepActions}>
                      <button style={S.btnNext}
                        onClick={e => { e.stopPropagation(); setCurrentStep(Math.min(i + 1, total - 1)); }}>
                        Étape suivante →
                      </button>
                      ...
                      {i > 0 && (
                        <button style={S.btnDone}
                          onClick={e => { e.stopPropagation(); setCurrentStep(i - 1); }}>
                          ← Retour
                        </button>
                      )}
                    </div>
...
export default function RoadmapPage({
  ...
  // Auto-ouvrir la première bourse
  useEffect(() => {
      ...
      <div style={S.layout}>
        ...
        <div style={S.left}>
          ...
          {user?.id && !boursesLoading && (
            <button style={S.btnRefresh} onClick={reload}>
              🔄 Actualiser mes bourses
            </button>
          )}
        </div>

        {/* ── Colonne droite : chat assistant ────────────────────────── */}
            ...
            {messages.length === 0 && (
              <div style={{ padding: 12 }}>
                ...
                {[
                  ...
                  <button key={i} style={S.suggestBtn} onClick={() => handleQuickReply(q)}>
                    {q}
                  </button>
                ))}
              </div>
...
const S = {
  ...
  bourseHeader:  { display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 22px', cursor: 'pointer', transition: 'background .2s', ':hover': { background: 'rgba(99,102,241,.05)' } },
  bourseInfo:    { flex: 1 },
