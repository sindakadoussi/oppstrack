...
/* ═══════════════════════════════════════════════════════════════════════════
   SUGGESTIONS (stockées en FR pour cohérence API)
═══════════════════════════════════════════════════════════════════════════ */
...
/* ═══════════════════════════════════════════════════════════════════════════
   TRADUCTIONS DES SUGGESTIONS (UI EN, stockage FR)
═══════════════════════════════════════════════════════════════════════════ */
const SUGGESTION_TRANSLATIONS = {
  countries: {
    'France': 'France', 'Canada': 'Canada', 'Belgique': 'Belgium', 'Suisse': 'Switzerland',
    'Allemagne': 'Germany', 'Italie': 'Italy', 'Espagne': 'Spain',
    'Royaume-Uni': 'United Kingdom', 'États-Unis': 'United States', 'Australie': 'Australia',
    'Suède': 'Sweden', 'Norvège': 'Norway', 'Danemark': 'Denmark',
    'Pays-Bas': 'Netherlands', 'Irlande': 'Ireland', 'Autriche': 'Austria',
    'Portugal': 'Portugal', 'Finlande': 'Finland',
    'Tunisie': 'Tunisia', 'Maroc': 'Morocco', 'Algérie': 'Algeria',
    'Sénégal': 'Senegal', 'Côte d\'Ivoire': 'Ivory Coast', 'Cameroun': 'Cameroon',
    'Liban': 'Lebanon', 'Égypte': 'Egypt'
  },
  fields: {
    'Informatique': 'Computer Science', 'Génie logiciel': 'Software Engineering',
    'Intelligence Artificielle': 'Artificial Intelligence', 'Data Science': 'Data Science',
    'Cybersécurité': 'Cybersecurity', 'Réseaux et Télécommunications': 'Networks & Telecommunications',
    'Systèmes embarqués': 'Embedded Systems',
    'Gestion': 'Management', 'Marketing': 'Marketing', 'Finance': 'Finance',
    'Comptabilité': 'Accounting', 'Ressources Humaines': 'Human Resources',
    'Droit': 'Law', 'Sciences politiques': 'Political Science',
    'Relations internationales': 'International Relations',
    'Médecine': 'Medicine', 'Pharmacie': 'Pharmacy', 'Biologie': 'Biology',
    'Chimie': 'Chemistry', 'Physique': 'Physics', 'Mathématiques': 'Mathematics',
    ...
    'Design': 'Design', 'Arts': 'Arts', 'Lettres': 'Literature',
    'Philosophie': 'Philosophy', 'Sociologie': 'Sociology', 'Psychologie': 'Psychology'
  },
  languages: {
    'Anglais': 'English', 'Français': 'French', 'Arabe': 'Arabic',
    'Espagnol': 'Spanish', 'Allemand': 'German', 'Italien': 'Italian',
    'Chinois': 'Chinese', 'Russe': 'Russian', 'Portugais': 'Portuguese',
    'Japonais': 'Japanese', 'Coréen': 'Korean', 'Néerlandais': 'Dutch',
    'Turc': 'Turkish', 'Polonais': 'Polish', 'Suédois': 'Swedish',
    'Grec': 'Greek', 'Vietnamien': 'Vietnamese', 'Hindi': 'Hindi'
  },
  skillCategories: {
    'language': lang => lang === 'fr' ? 'Langage de programmation' : 'Programming language',
    'framework': lang => lang === 'fr' ? 'Framework / Librairie' : 'Framework / Library',
    'database': lang => lang === 'fr' ? 'Base de données' : 'Database',
    'tool': lang => lang === 'fr' ? 'Outil / Logiciel' : 'Tool / Software',
    'devops': lang => lang === 'fr' ? 'DevOps / Cloud' : 'DevOps / Cloud',
    'method': lang => lang === 'fr' ? 'Méthode / Agilité' : 'Method / Agile',
    'design': lang => lang === 'fr' ? 'Design / UI' : 'Design / UI',
    'other': lang => lang === 'fr' ? 'Autre' : 'Other'
  },
  skillLevels: {
    'beginner': lang => lang === 'fr' ? 'Débutant' : 'Beginner',
    'intermediate': lang => lang === 'fr' ? 'Intermédiaire' : 'Intermediate',
    'advanced': lang => lang === 'fr' ? 'Avancé' : 'Advanced',
    'expert': lang => lang === 'fr' ? 'Expert' : 'Expert'
  },
  degreeLevels: {
    'Licence 1': 'Bachelor Year 1', 'Licence 2': 'Bachelor Year 2', 'Licence 3': 'Bachelor Year 3',
    'Master 1': 'Master Year 1', 'Master 2': 'Master Year 2',
    'Doctorat': 'PhD',
    'Ingénieur 1': 'Engineering Year 1', 'Ingénieur 2': 'Engineering Year 2', 'Ingénieur 3': 'Engineering Year 3'
  },
  projectTypes: {
    'pfe': lang => lang === 'fr' ? 'Projet de Fin d\'Études (PFE)' : 'Final Year Project',
    'academic': lang => lang === 'fr' ? 'Projet de cours / académique' : 'Course / academic project',
    'personal': lang => lang === 'fr' ? 'Projet personnel' : 'Personal project',
    'entrepreneurial': lang => lang === 'fr' ? 'Projet entrepreneurial' : 'Entrepreneurial project',
    'research': lang => lang === 'fr' ? 'Projet de recherche' : 'Research project'
  },
  teamSizes: {
    'solo': lang => lang === 'fr' ? 'Projet individuel' : 'Individual project',
    '2': '2 people', '3-5': '3-5 people', '6+': lang => lang === 'fr' ? 'Plus de 5 personnes' : 'More than 5 people'
  },
  workTypes: {
    'internship': lang => lang === 'fr' ? 'Stage' : 'Internship',
    'job': lang => lang === 'fr' ? 'Emploi' : 'Job',
    'freelance': 'Freelance'
  }
};
...
/* ═══════════════════════════════════════════════════════════════════════════
   COMPOSANT AUTOCOMPLETE (avec suggestions traduites)
═══════════════════════════════════════════════════════════════════════════ */
function AutocompleteInput({ value, onChange, suggestions, placeholder, label, type = 'text', category = null }) {
  ...
  const handleInputChange = (e) => {
    ...
    if (originalMatch) {
      ...
      onChange(originalMatch); // Stocke la valeur FR
    } else {
  ...
  const handleSelectSuggestion = (originalValue) => {
    ...
    onChange(originalValue); // Stocke FR
...
/* ═══════════════════════════════════════════════════════════════════════════
   COMPOSANT SKILL INPUT (avec traductions)
═══════════════════════════════════════════════════════════════════════════ */
  ...
  return (
    <div style={S.page}>
      <div style={S.body}>
        ...
        {tab === 'personal' && (
          <div style={S.sec}>
            <T>{lang === 'fr' ? 'Informations personnelles' : 'Personal information'}</T>
            <div style={S.g2}>
            ...
            <T>{lang === 'fr' ? 'Liens professionnels' : 'Professional links'}</T>
            <div style={S.g2}>
