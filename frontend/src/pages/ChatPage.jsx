...
export default function ChatPage({user,messages,input,setInput,loading,handleSend,handleQuickReply,chatContainerRef,setView,bourses=[],appliedNoms,onApplyBourse}){
      ...
      if (isStarred) {
        ...
        if (favDoc?.id) {
          ...
          await axiosInstance.post('/api/favoris', {
            user: user.id, userEmail: user.email || '', bourses: [bourseData]
          });
        }
  ...
  const handleApply = useCallback(async (bourse) => {
    ...
    try {
      ...
      try {
        ...
        console.warn('Webhook roadmap non disponible', webhookErr);
      }
  ...
  return(
    <div className="chat-page">
      ...
      {/* ═══ HERO + CHAT avec background photo ═══ */}
      <div className="hero-bg-section">
        ...
        <div className="chat-hero">
          <h1 className="hero-title">
            {lang==='fr'?'Trouvez votre bourse':'Find your scholarship'}
            <br/>
            <span className="hero-accent">{lang==='fr'?'100% financée':'100% funded'}</span>
          </h1>
          <p className="hero-sub">
            {lang==='fr'
              ?'Discutez avec notre IA. Elle analyse votre profil, recommande les meilleures opportunités et vous guide à chaque étape.'
              :'Chat with our AI. It analyzes your profile, recommends the best opportunities and guides you every step of the way.'}
          </p>
        </div>
        ...
        <div className="chat-box">
            ...
            {messages.length===0&&(
              <div className="welcome-screen">
                <div className="welcome-avatar">
                  <img src="/logo.png" alt="OppsTrack" style={{width:36,height:36,objectFit:'contain',borderRadius:6}} onError={e=>{e.target.style.display='none';e.target.parentNode.innerHTML='🤖';}}/>
                </div>
                <div className="welcome-bubble">
                  <p><strong>{lang==='fr'?'Bonjour':'Hello'}{user?.name?` ${user.name}`:''}!</strong> 👋</p>
                  <p>{lang==='fr'?'Je suis votre assistant OppsTrack. Je peux :':'I am your OppsTrack assistant. I can:'}</p>
                  <ul>
                    <li>{lang==='fr'?'🎓 Recommander des bourses selon votre profil':'🎓 Recommend scholarships matching your profile'}</li>
                    <li>{lang==='fr'?'📋 Vous guider sur les démarches à suivre':'📋 Guide you through the application process'}</li>
                    <li>{lang==='fr'?'🎙️ Vous préparer à vos entretiens':'🎙️ Prepare you for scholarship interviews'}</li>
                    <li>{lang==='fr'?'📄 Analyser votre CV et lettre de motivation':'📄 Analyze your CV and cover letter'}</li>
                  </ul>
                  <p>{lang==='fr'?'Par où voulez-vous commencer ?':'Where would you like to start?'}</p>
                </div>
              </div>
            )}
            {messages.map((msg,i)=>(
              <ChatMessage key={i} msg={msg} index={i} appliedNoms={appliedSet} onApply={handleApply}
                onDetails={setDrawer} allBourses={bourses} handleQuickReply={handleQuickReply} lang={lang}/>
            ))}
            {loading&&(
              <div className="msg ai">
                <div className="msg-avatar">🤖</div>
                <div className="msg-bubble typing-bubble"><span className="dot"/><span className="dot"/><span className="dot"/></div>
              </div>
            )}
          ...
          <div className="quick-replies">
            {quickReplies.map((qr,i)=>(
              <button key={i} className="quick-reply-btn" onClick={()=>handleQuickReply(qr.text)} disabled={loading}>
      ...
      <div className="hero-stats">
        <div className="stat">
          ...
          <span className="stat-label">{lang==='fr'?'Bourses':'Scholarships'}</span>
        </div>
        <div className="stat-divider"/>
        <div className="stat">
          <StatNumber value={heroStats.pctFinancees} suffix="%" loading={!heroStats.loaded}/>
          <span className="stat-label">{lang==='fr'?'Financées':'Funded'}</span>
        </div>
        <div className="stat-divider"/>
        <div className="stat">
          <span className="stat-num">24/7</span>
          <span className="stat-label">{lang==='fr'?'IA active':'AI active'}</span>
        </div>
      </div>
      ...
      <div className="features-section">
        <div className="features-header">
          <div className="features-eyebrow">
            ...
            {lang==='fr'?'Pourquoi OppsTrack ?':'Why OppsTrack?'}
            <span className="features-eyebrow-line"/>
          </div>
          <h2 className="features-title">
            {lang==='fr'?'Tout ce dont vous avez besoin pour':'Everything you need to'}
            <span className="features-title-gradient"> {lang==='fr'?'décrocher votre bourse':'land your scholarship'}</span>
          </h2>
        </div>
        <div className="features-grid">
          {[
            {icon:'🔍',titleFr:'Matching intelligent',titleEn:'Smart Matching',descFr:"L'IA analyse votre profil et recommande les bourses avec les meilleures chances de succès.",descEn:"The AI analyzes your profile and recommends scholarships with the best success chances.",ctaFr:'Trouver mes bourses',ctaEn:'Find my scholarships',view:'bourses',accent:'#255cae',bg:'#eff6ff'},
            {icon:'📋',titleFr:'Roadmap personnalisée',titleEn:'Personalized Roadmap',descFr:"Chaque candidature décomposée étape par étape : documents, lettre, soumission, résultat.",descEn:"Each application broken down step by step: documents, letter, submission, result.",ctaFr:'Voir la roadmap',ctaEn:'See the roadmap',view:'roadmap',accent:'#166534',bg:'#f0fdf4'},
            {icon:'🎙️',titleFr:'Entretiens simulés IA',titleEn:'AI Mock Interviews',descFr:"Notre IA joue le rôle du jury. Obtenez un score et des conseils personnalisés.",descEn:"Our AI plays the role of the jury. Get a score and personalized feedback.",ctaFr:'Préparer mon entretien',ctaEn:'Prepare my interview',view:'entretien',accent:'#f5a623',bg:'#fffbeb'},
            {icon:'📄',titleFr:'Analyse de documents',titleEn:'Document Analysis',descFr:"CV, lettre de motivation — l'IA identifie vos points forts et propose des améliorations.",descEn:"CV, cover letter — the AI identifies your strengths and suggests improvements.",ctaFr:'Analyser mon CV',ctaEn:'Analyze my CV',view:'cv',accent:'#0891b2',bg:'#ecfeff'},
            {icon:'🌍',titleFr:'Carte mondiale',titleEn:'World Map',descFr:"Explorez les bourses dans le monde entier. Filtrez par pays, niveau et domaine.",descEn:"Explore scholarships worldwide. Filter by country, level and field.",ctaFr:'Explorer la carte',ctaEn:'Explore the map',view:'bourses',accent:'#7c3aed',bg:'#f5f3ff'},
            {icon:'📊',titleFr:'Tableau de bord',titleEn:'Dashboard',descFr:"Suivez vos candidatures, alertes de deadlines et score de préparation en temps réel.",descEn:"Track your applications, deadline alerts and preparation score in real time.",ctaFr:'Voir le dashboard',ctaEn:'See the dashboard',view:'dashboard',accent:'#dc2626',bg:'#fef2f2'},
          ].map((feat,i)=>(
            <div key={i} className="feat-card" style={{animationDelay:`${i*0.07}s`}}
              onClick={()=>setView&&setView(feat.view)} role="button" tabIndex={0}
              onKeyDown={e=>e.key==='Enter'&&setView&&setView(feat.view)}>
              <div className="feat-icon-circle" style={{background:feat.bg,color:feat.accent}}>{feat.icon}</div>
              <h3 className="feat-title">{lang==='fr'?feat.titleFr:feat.titleEn}</h3>
              <p className="feat-desc">{lang==='fr'?feat.descFr:feat.descEn}</p>
              <div className="feat-cta" style={{color:feat.accent}}>
                {lang==='fr'?feat.ctaFr:feat.ctaEn}
                <svg className="feat-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M5 12h14M12 5l7 7-7 7"/></svg>
              </div>
            ...
            <div className="testimonials-row row-left">
              {feedbacks.slice(0, Math.ceil(feedbacks.length / 2)).map((fb) => (
                <div key={fb.id} className="testimonial-card">
                  <div className="testimonial-stars">{'★'.repeat(fb.rating)}{'☆'.repeat(5 - fb.rating)}</div>
                  <p className="testimonial-comment">"{fb.comment.length > 180 ? `${fb.comment.slice(0, 180)}…` : fb.comment}"</p>
                  <div className="testimonial-author">
                    <strong>{fb.name}</strong>
                    <span className="testimonial-date">
                      {fb.createdAt ? new Date(fb.createdAt).toLocaleDateString(lang === 'fr' ? 'fr-FR' : 'en-US', {year:'numeric',month:'short',day:'numeric'}) : (lang === 'fr' ? 'Récent' : 'Recent')}
                    </span>
                  </div>
                </div>
              ))}
      ...
      <style>{`/* ── PAGE ── */
.chat-page{display:flex;flex-direction:column;align-items:center;width:100%;max-width:800px;margin:0 auto;padding:0 16px 32px;font-family:'Segoe UI',system-ui,sans-serif}
...
.hero-bg-section::before{
  ...
  position: absolute;
  inset: 0;
  background-image: url('/tunisie.jpg');
...
.hero-bg-overlay{
  position: absolute;
  inset: 0;
  background: rgba(0, 0, 0, 0.5); /* ← Assombrissement neutre, sans couleur */
  z-index: 1;
}

.hero-bg-section > *:not(.hero-bg-overlay){
  position: relative;
  z-index: 2;
  max-width: 800px;
  margin: 0 auto;
...
/* ── HERO TEXT ── */
.chat-hero{text-align:center;margin-bottom:24px;width:100%}
.hero-title{font-size:clamp(1.8rem,5vw,2.8rem);font-weight:800;line-height:1.2;color:#ffffff;margin-bottom:14px;letter-spacing:-.02em;text-shadow:0 2px 12px rgba(0,0,0,0.3)}
.hero-accent{background:linear-gradient(135deg,#f5a623,#ffd166);-webkit-background-clip:text;-webkit-text-fill-color:transparent;background-clip:text}
.hero-sub{color:rgba(255,255,255,0.78);font-size:15px;max-width:520px;margin:0 auto;line-height:1.6}

/* ── CHAT BOX — CADRE BLEU TRANSPARENT FONCÉ ── */
.chat-box {
  width: 100%;
  max-width: 720px;
  margin: 0 auto;
  background: transparent;          /* ← pas de couleur, juste le flou */
  backdrop-filter: blur(16px);      /* ← flou plus fort (ajustez selon goût) */
  -webkit-backdrop-filter: blur(16px);
  border-radius: 16px 16px 0 0;
  border: 1px solid rgba(255, 255, 255, 0.2);
  border-top: 3px solid rgba(245, 166, 35, 0.6);
  box-shadow: 0 -8px 32px rgba(0, 0, 0, 0.2);
  overflow: hidden;
}
/* Forcer la zone d'input à être transparente avec l'effet glassy */
.chat-input-glass {
...
/* Cibler tous les champs de saisie possibles à l'intérieur */
.chat-input-glass input,
.chat-input-glass textarea,
.chat-input-glass .chat-input-field,
.chat-input-glass div[contenteditable="true"],
.chat-input-glass > div,
.chat-input-glass form {
  background: transparent !important;
  backdrop-filter: blur(16px);
  -webkit-backdrop-filter: blur(16px);
...
/* Style du bouton d'envoi (garder un fond légèrement opaque pour le contraste) */
.chat-input-glass button {
  background: rgba(37, 92, 174, 0.9);
...
.chat-input-glass button:hover {
  background: rgba(37, 92, 174, 1);
...
.welcome-bubble li{margin-bottom:4px;color:#334155}
...
.msg-avatar-user{
  background:rgba(37, 92, 174, 0.95);
...
@keyframes dotBounce{
  0%,60%,100%{transform:scale(.7);opacity:.5}
...
.bc-grid-title{font-size:14px;font-weight:700;color:#fff}
.bc-grid-sub{
  margin-left:auto;
  font-size:11px;
...
.bourse-card-0 .bourse-card-bar{background:linear-gradient(90deg,#255cae,#2563eb)}
...
.bc-title{font-size:13px;font-weight:700;color:#0f172a;margin:0 0 3px;line-height:1.35;transition:color .15s}
...
.testimonials-section{
  ...
  padding: 64px 24px;
  overflow: hidden;
  background: linear-gradient(135deg, #1a3a6b 0%, #255cae 50%, #1a3a6b 100%);
  position: relative;
  margin-top: 40px;
}
...
@media(max-width:420px){
  ...
  .testimonials-row{flex-direction:column;align-items:center;animation:none!important}
