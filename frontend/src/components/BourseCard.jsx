...
const daysLeft = (deadline, lang = 'fr') => {
  ...
  if (diff < 0)   return { label: lang === 'fr' ? 'Expirée'        : 'Expired', color: '#ef4444' };
  if (diff === 0) return { label: lang === 'fr' ? "Aujourd'hui !"  : 'Today!',  color: '#ef4444' };
  if (diff <= 30) return { label: `${diff} ${lang === 'fr' ? 'jours' : 'days'}`, color: '#f97316' };
  return           { label: `${diff} ${lang === 'fr' ? 'jours' : 'days'}`, color: '#16a34a' };
};
...
const formatDate = (d, lang = 'fr') => {
  if (!d) return null;
  try { return new Date(d).toLocaleDateString(lang === 'fr' ? 'fr-FR' : 'en-US', { day: 'numeric', month: lang === 'fr' ? 'long' : 'short', year: 'numeric' }); }
  catch { return d; }
};
...
export default function BourseCard({ bourse, user, onAskAI, onClick, starred, onStar, applied, onApply }) {
  ...
  // ✅ Utiliser TranslationProvider — DeepL traduit tout automatiquement
  const { trBourse } = usePageTranslation(bourse, 'bourse');
  const b = trBourse(bourse); // bourse traduite (ou originale si lang=fr / pas encore traduit)

  const pct        = calcMatch(bourse, user);
  const scoreColor = getScoreColor(pct);
  const dl         = daysLeft(bourse.dateLimite, lang);
  const urgent     = isUrgent(bourse.dateLimite);
  const niveaux    = b.niveau ? b.niveau.split(',').map(s => s.trim()).filter(Boolean) : [];
  ...
  const t = {
    ...
    roadmap:     lang === 'fr' ? '✅ Roadmap'         : '✅ In roadmap',
    apply:       lang === 'fr' ? '🗺️ Postuler'       : '🗺️ Apply',
    favorite:    lang === 'fr' ? 'Favori'             : 'Favorite',
    askAI:       lang === 'fr' ? "Demander à l'IA"   : 'Ask AI',
    international: lang === 'fr' ? 'International'   : 'International',
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ padding:'14px 16px 10px', display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:8, cursor:'pointer' }} onClick={onClick}>
        <div style={{ display:'flex', alignItems:'center', gap:8, flex:1, minWidth:0 }}>
          ...
          <div style={{ minWidth:0 }}>
            <div style={{ fontSize:11, color:'#64748b', fontWeight:500, display:'flex', alignItems:'center', gap:6 }}>
              {b.pays || t.international}
              {urgent && <span style={{ fontSize:9, padding:'1px 6px', borderRadius:4, background:'#fef2f2', color:'#ef4444', fontWeight:700, textTransform:'uppercase', border:'1px solid #fecaca' }}>{t.urgent}</span>}
            </div>
        ...
        {pct !== null && (
          <div style={{ textAlign:'right', flexShrink:0 }}>
            ...
            <div style={{ fontSize:10, color:'#64748b', marginTop:2 }}>{getScoreLabel(pct, lang)}</div>
          </div>
        )}
      </div>
      ...
      <div style={{ padding:'12px 16px', flex:1, display:'flex', flexDirection:'column', gap:8, cursor:'pointer' }} onClick={onClick}>
        ...
        {b.financement && (
          <div style={{ display:'flex', alignItems:'flex-start', gap:8 }}>
            ...
            <span style={{ fontSize:12, color:'#475569', lineHeight:1.5 }}>{b.financement}</span>
          </div>
        )}
        {b.description && (
        ...
        {(formatDate(bourse.dateLimite, lang) || dl) && (
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:2, padding:'8px 10px', borderRadius:6, background:'#f8fafc', border:'1px solid #f1f5f9' }}>
            {formatDate(bourse.dateLimite, lang) && (
              <div>
                <div style={{ fontSize:10, color:'#94a3b8', textTransform:'uppercase', letterSpacing:'0.05em', marginBottom:2 }}>{t.deadline}</div>
                <div style={{ fontSize:12, color:dl?.color || '#475569', fontWeight:700 }}>{formatDate(bourse.dateLimite, lang)}</div>
              </div>
            )}
            {dl && (
      ...
      <div style={{ padding:'10px 16px 14px', display:'flex', gap:6, borderTop:'1px solid #f1f5f9' }}>
        <button style={{ flex:1, padding:'9px 10px', borderRadius:6, background:'#1a3a6b', border:'none', color:'#fff', fontSize:12, fontWeight:600, cursor:'pointer' }}
        ...
        <button
          ...
          {applyLoading ? '⏳' : applied ? t.roadmap : t.apply}
        </button>
        <button
          ...
          disabled={starLoading} title={t.favorite}>
          {starLoading ? '⏳' : starred ? '★' : '☆'}
        </button>
        <button
          ...
          onClick={e => { e.stopPropagation(); onAskAI(bourse); }} title={t.askAI}>
          🤖
        </button>
      </div>
    </div>
