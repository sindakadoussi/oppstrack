...
const daysLeft = (deadline, lang = 'fr') => {
  ...
  const labels = {
    expired: lang === 'fr' ? 'Expirée' : 'Expired',
    today: lang === 'fr' ? "Aujourd'hui !" : 'Today!',
    days: lang === 'fr' ? 'jours' : 'days',
    remaining: lang === 'fr' ? 'restants' : 'remaining'
  };
  
  if (diff < 0)   return { label: labels.expired, color: '#ef4444' };
  if (diff === 0) return { label: labels.today,  color: '#ef4444' };
  if (diff <= 30) return { label: `${diff} ${labels.days}`,  color: '#f97316' };
  return { label: `${diff} ${labels.days}`, color: '#16a34a' };
};
...
export default function BourseCard({ bourse, user, onAskAI, onClick, starred, onStar, applied, onApply }) {
  ...
  const t = {
    ...
    noDeadline: lang === 'fr' ? 'Pas de date limite' : 'No deadline',
  };
  ...
  {tCountry(bourse.pays, lang) || t.international}
              {urgent && (
      ...
      <div style={{ padding:'12px 16px', flex:1, display:'flex', flexDirection:'column', gap:8, cursor:'pointer' }} onClick={onClick}>
        {niveaux.length > 0 && (
          <div style={{ display:'flex', flexWrap:'wrap', gap:5 }}>
            {niveaux.map((n, i) => (
  <span key={i} style={{ fontSize:11, padding:'2px 8px', borderRadius:4, background:'#eff6ff', border:'1px solid #bfdbfe', color:'#1a3a6b', fontWeight:500 }}>
    {tLevel(n, lang)}  {/* ✅ Traduit */}
  </span>
))}
          </div>
        )}
  ...
  {tFunding(bourse.financement, lang)}  {/* ✅ Traduit */}
</span>
          </div>
        )}
        {bourse.description && (
